...
def bucket_finding(
    ...
    if status == "rejected":
        return (
            "false_positive",
            f"Rejected by Validate: {f.get('validate_reason', 'insufficient evidence')}",
        )

    if is_api_by_design(
        f,
        {"name": f.get("function_name", ""), "content": f.get("desc", "")},
    ):
    ...
    if trace_required and requires_trace_before_fix_now(True, trace_confirmed):
        if sev in {"CRITICAL", "HIGH"} and status == "confirmed":
            return (
                "backlog",
                "Library target requires consumer Trace confirmation before fix_now",
            )

    if sev in {"CRITICAL", "HIGH"} and status == "confirmed":
        return (
            "fix_now",
            f"Severity {sev} + status {status}. Confirmed reachable vulnerability.",
        )

    return (
        "backlog",
        f"Severity {sev}, status {status}. Needs further evidence or is lower priority.",
    )
...
def build_report(
    ...
    if exposure_metrics:
        summary["avg_exposure_window_days"] = float(
            exposure_metrics.get("avg_exposure_window_days", 0.0),
        )
        summary["median_exposure_window_days"] = float(
            exposure_metrics.get("median_exposure_window_days", 0.0),
        )
        summary["oldest_open_exposure_days"] = float(
            exposure_metrics.get("oldest_open_exposure_days", 0.0),
        )

    return {
        "repo": repo,
        "scan_date": datetime.now(timezone.utc).isoformat(),  # noqa: UP017 — from datetime import datetime makes datetime.UTC invalid
        "bucket_definitions": {
