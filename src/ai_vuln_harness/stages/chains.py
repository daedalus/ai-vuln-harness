...
def _mitigations(finding_a: dict, finding_b: dict) -> list[str]:
    classes = {
        str(finding_a.get("class", "")).lower(),
        str(finding_b.get("class", "")).lower(),
    }
...
def _should_chain(
    ...
    if not (cross_component or severe):
        return False, "", "", "", ""
    return True, a_file, b_file, comp_a, comp_b
...
def _build_chain(
    ...
    return {
        "chain_id": chain_id,
        "feasible": bool(
            a.get("call_path_verified", True) and b.get("call_path_verified", True),
        ),
        ...
        "cvss": {
            ...
            if cross_component
            else "AV:N/AC:L/PR:L/UI:N/S:U/C:H/I:H/A:H",
            "score": cvss,
        },
        "narrative": (
...
def synthesize_exploit_chains(
    ...
    confirmed.sort(
        key=lambda f: _SEV_RANK.get(str(f.get("severity", "")).upper(), 0),
        reverse=True,
    )
    ...
    for i, a in enumerate(confirmed):
        for b in confirmed[i + 1 :]:
            ok, a_file, b_file, comp_a, comp_b = _should_chain(a, b, by_id, seen_pairs)
            if not ok:
                continue
            chain = _build_chain(
                a,
