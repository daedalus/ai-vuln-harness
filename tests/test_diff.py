...
from ai_vuln_harness.stages.diff import (
...
class GetChangedSnippetsTests(unittest.TestCase):
    def _mock_ranges(self, ranges: dict):
        return patch(
            "ai_vuln_harness.stages.diff.get_changed_line_ranges", return_value=ranges
        )
    ...
    def test_head_commit_defaults_to_HEAD(self):
        ...
        with patch(
            "ai_vuln_harness.stages.diff.get_changed_line_ranges",
            side_effect=mock_ranges,
        ):
