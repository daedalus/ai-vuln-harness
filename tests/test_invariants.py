...
CONFIG_DIR = Path(__file__).parent.parent / "src" / "ai_vuln_harness" / "config"
SCHEMAS_DIR = Path(__file__).parent.parent / "src" / "ai_vuln_harness" / "schemas"
PROMPTS_DIR = Path(__file__).parent.parent / "src" / "ai_vuln_harness" / "prompts"
...
class StructuralInvariantTests(unittest.TestCase):
    ...
    def test_run_py_is_entry_point(self):
        run_py = Path(__file__).parent.parent / "src" / "ai_vuln_harness" / "run.py"
    ...
    def test_config_files_exist(self):
        ...
        self.assertTrue(
            (CONFIG_DIR / "stages.json").exists(), "Missing config/stages.json"
        )

    def test_prompts_exist(self):
        for prompt in ["hunt.md", "recon.md", "validate.md", "trace.md", "report.md"]:
            self.assertTrue(
                (PROMPTS_DIR / prompt).exists(), f"Missing prompt: {prompt}"
            )
    ...
    def test_schemas_exist(self):
        for schema in [
            ...
            "finding.schema.json",
            "recon_tasks.schema.json",
            "report.schema.json",
        ]:
            self.assertTrue(
                (SCHEMAS_DIR / schema).exists(), f"Missing schema: {schema}"
            )
...
class IngestorInvariantTests(unittest.TestCase):
    def test_snippet_id_deterministic(self):
        from ai_vuln_harness.stages.ingestor import _make_snippet_id
    ...
    def test_snippet_id_format(self):
        ...
        self.assertTrue(
            sid.startswith("sha256:"), f"ID does not start with sha256: {sid}"
        )
    ...
    def test_snippet_required_fields_defined(self):
        from ai_vuln_harness.stages.ingestor import SNIPPET_REQUIRED_FIELDS
...
class CoordinatorInvariantTests(unittest.TestCase):
    def test_exactly_11_domains(self):
        from ai_vuln_harness.stages.coordinator import DOMAIN_ORDER

        self.assertEqual(
            len(DOMAIN_ORDER), 11, f"Expected 11 domains, got {len(DOMAIN_ORDER)}"
        )
    ...
    def test_domains_have_exclusive_flag(self):
        from ai_vuln_harness.stages.coordinator import DOMAINS

        for d in DOMAINS:
            self.assertIn(
                "exclusive", d, f"Domain {d.get('name')} missing exclusive flag"
            )
...
class ChainInvariantTests(unittest.TestCase):
    def test_filter_unreachable_accepts_entry_points(self):
        from ai_vuln_harness.stages.shield import filter_unreachable
    ...
    def test_call_graph_keyed_lowercase(self):
        from ai_vuln_harness.stages.shield import build_call_graph
    ...
    def test_call_path_normalized_to_list_at_parse_time(self):
        ...
        text = (
            '[{"snippet_id": "x", "severity": "HIGH", "class": "buf", '
...
class PipelineInvariantTests(unittest.TestCase):
    def test_stage_order_constant(self):
        from ai_vuln_harness.stages.contracts import PIPELINE_STAGES
    ...
    def test_output_dir_is_creatable(self):
...
class QualityGateInvariantTests(unittest.TestCase):
    ...
    def test_each_finding_passes_shield(self):
        ...
        snippet_db = {
            "s1": {"content": "void a() { b(); }", "callers": [], "callees": ["b"]}
        }
        graph = {"a": {"b"}, "b": set()}
